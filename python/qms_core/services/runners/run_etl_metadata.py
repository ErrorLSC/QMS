...
def main(dry_run=False):
    ...
    for name in ["ILM", "IWI", "IIM", "DPS", "AVM","GCC"]
    }

    MetadataPipeline = create_config_driven_pipeline(
        namespace="metadata",
        run_order=["ILM", "IWI", "IIM", "DPS", "AVM","GCC"]
    )
    pipeline = MetadataPipeline(config=config, job_configs=job_configs)
